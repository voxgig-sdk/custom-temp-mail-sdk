...
describe('PublicV1DashboardAnalyticsDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CustomTempMailSDK({
  ...
  test('direct-load-public_v1_dashboard_analytics', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-public_v1_dashboard_analytics', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOM_TEMP_MAIL_TEST_PUBLIC_V1_DASHBOARD_ANALYTICS_ENTID']
