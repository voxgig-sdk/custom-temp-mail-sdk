...
describe('CustomDomainDirect', async () => {
  ...
  test('direct-list-custom_domain', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-custom_domain', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOM_TEMP_MAIL_TEST_CUSTOM_DOMAIN_ENTID']
