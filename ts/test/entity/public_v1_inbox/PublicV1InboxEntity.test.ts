...
describe('PublicV1InboxEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'remove']) {
      if (maybeSkipControl(t, 'entityOp', 'public_v1_inbox.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set CUSTOM_TEMP_MAIL_TEST_PUBLIC_V__INBOX_ENTID JSON to run live')
      return
    }
    ...
    public_v1_inbox_ref01_data = await public_v1_inbox_ref01_ent.create(public_v1_inbox_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CUSTOM_TEMP_MAIL_TEST_PUBLIC_V__INBOX_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'CUSTOM_TEMP_MAIL_TEST_PUBLIC_V__INBOX_ENTID': idmap,
    'CUSTOM_TEMP_MAIL_TEST_LIVE': 'FALSE',
    'CUSTOM_TEMP_MAIL_TEST_EXPLAIN': 'FALSE',
    'CUSTOM_TEMP_MAIL_APIKEY': 'NONE',
  })

  idmap = env['CUSTOM_TEMP_MAIL_TEST_PUBLIC_V__INBOX_ENTID']
