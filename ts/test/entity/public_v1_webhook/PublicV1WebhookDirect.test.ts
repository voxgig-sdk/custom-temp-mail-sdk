...
describe('PublicV1WebhookDirect', async () => {
  ...
  test('direct-list-public_v1_webhook', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-public_v1_webhook', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOM_TEMP_MAIL_TEST_PUBLIC_V1_WEBHOOK_ENTID']
