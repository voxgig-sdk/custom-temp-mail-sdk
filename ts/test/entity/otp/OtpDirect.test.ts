...
describe('OtpDirect', async () => {
  ...
  test('direct-load-otp', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-otp', setup.live)) return
    if (skipIfMissingIds(t, setup, ["inbox_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOM_TEMP_MAIL_TEST_OTP_ENTID']
