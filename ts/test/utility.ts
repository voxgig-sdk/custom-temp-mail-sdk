...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.CUSTOMTEMPMAIL_TEST_LIVE ||
    'TRUE' === process.env.CUSTOMTEMPMAIL_TEST_OVERRIDE
  ) {
  ...
  m.CUSTOMTEMPMAIL_TEST_EXPLAIN = process.env.CUSTOMTEMPMAIL_TEST_EXPLAIN || m.CUSTOMTEMPMAIL_TEST_EXPLAIN
