...
const __1 = require("..");
const SDK_NAME = 'CustomTempMailSDK';
...
const TEST_SEED = { "entity": { "custom_domain": { "test01": { "id": "test01" } }, "custom_domain_verify": { "test01": { "id": "test01" } }, "domain": { "test01": { "id": "test01" } }, "domains_all": { "test01": { "id": "test01" } }, "inbox": { "test01": { "id": "test01" } }, "men": { "test01": { "id": "test01" } }, "message": { "test01": { "id": "test01" } }, "otp": { "test01": { "id": "test01" } }, "plan": { "test01": { "id": "test01" } }, "public_v1_dashboard_analytics": { "test01": { "id": "test01" } }, "public_v1_inbox": { "test01": { "id": "test01" } }, "public_v1_message": { "test01": { "id": "test01" } }, "public_v1_webhook": { "test01": { "id": "test01" } }, "usage": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+CustomTempMailSDK\b/.test(code) ||
        /\bCustomTempMailSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CustomTempMailSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CustomTempMailSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
