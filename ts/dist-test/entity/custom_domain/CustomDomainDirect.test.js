...
(0, node_test_1.describe)('CustomDomainDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-custom_domain', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-custom_domain', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CUSTOM_TEMP_MAIL_TEST_CUSTOM_DOMAIN_ENTID'];
