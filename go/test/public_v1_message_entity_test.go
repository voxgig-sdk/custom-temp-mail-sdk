...
func TestPublicV1MessageEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMTEMPMAIL_TEST_PUBLIC_V__MESSAGE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if publicV1MessageRef01DataDt0Loaded == nil {
...
func public_v1_messageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMTEMPMAIL_TEST_PUBLIC_V__MESSAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMTEMPMAIL_TEST_PUBLIC_V__MESSAGE_ENTID"])
