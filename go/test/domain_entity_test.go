...
func TestDomainEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMTEMPMAIL_TEST_DOMAIN_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func domainBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMTEMPMAIL_TEST_DOMAIN_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"CUSTOMTEMPMAIL_TEST_DOMAIN_ENTID": idmap,
		"CUSTOMTEMPMAIL_TEST_LIVE":      "FALSE",
		"CUSTOMTEMPMAIL_TEST_EXPLAIN":   "FALSE",
		"CUSTOMTEMPMAIL_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["CUSTOMTEMPMAIL_TEST_DOMAIN_ENTID"])
