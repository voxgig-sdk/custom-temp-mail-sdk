...
import (
	...
	omni "github.com/voxgig-sdk/custom-temp-mail-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CustomTempMailError); is {
				out := map[string]any{
					"name":    "CustomTempMailError",
...
func omniCtx(arg any, client *sdk.CustomTempMailSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CustomTempMailSDK); is {
			client = live
