...
func TestPublicV1WebhookEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMTEMPMAIL_TEST_PUBLIC_V__WEBHOOK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		publicV1WebhookRef01Data = core.ToMapAny(publicV1WebhookRef01DataResult)
...
func public_v1_webhookBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMTEMPMAIL_TEST_PUBLIC_V__WEBHOOK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMTEMPMAIL_TEST_PUBLIC_V__WEBHOOK_ENTID"])
