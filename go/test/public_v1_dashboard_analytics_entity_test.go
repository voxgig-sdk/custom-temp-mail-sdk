...
func TestPublicV1DashboardAnalyticsEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMTEMPMAIL_TEST_PUBLIC_V__DASHBOARD_ANALYTICS_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func public_v1_dashboard_analyticsBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMTEMPMAIL_TEST_PUBLIC_V__DASHBOARD_ANALYTICS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMTEMPMAIL_TEST_PUBLIC_V__DASHBOARD_ANALYTICS_ENTID"])
