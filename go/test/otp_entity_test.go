...
func TestOtpEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMTEMPMAIL_TEST_OTP_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		otpRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.otp", setup.data)))
...
func otpBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMTEMPMAIL_TEST_OTP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMTEMPMAIL_TEST_OTP_ENTID"])
