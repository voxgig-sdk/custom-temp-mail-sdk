...
// CustomTempMail SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class CustomTempMailMakePoint
{
    public static function call(CustomTempMailContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof CustomTempMailError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = CustomTempMailHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = CustomTempMailHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
