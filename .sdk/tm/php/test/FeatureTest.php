...
// CustomTempMail SDK feature test
//
...
class FtCtrl extends CustomTempMailControl
{
...
class FtClient
{
    ...
    private CustomTempMailUtility $utility;

    public function __construct(CustomTempMailUtility $utility, array $options)
    {
    ...
    public function get_utility(): CustomTempMailUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CustomTempMailContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CustomTempMailContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CustomTempMailContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CustomTempMailConfig::make_feature($name);
    ...
    private static function buildUrl(CustomTempMailSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CustomTempMailError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CustomTempMailResponse($fetched);
        }
    ...
    private function fail(CustomTempMailContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CustomTempMailContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
