# CustomTempMail SDK utility: fetcher
...
module CustomTempMailUtilities
  ...
  DefaultHttpFetch = ->(fullurl, fetchdef) {
    ...
    begin
      ...
      request['User-Agent'] = 'Mozilla/5.0 (compatible; CustomTempMailSDK/1.0)' unless has_ua
      request.body = body_str if body_str.is_a?(String)

      http = CustomTempMailUtilities.http_checkout(key, uri, proxy)
      begin
        # Net::HTTP retries idempotent requests; an EOF can follow a committed POST.
        resp = http.request(request)
      rescue StandardError
        CustomTempMailUtilities.http_discard(http)
        raise
      end
      CustomTempMailUtilities.http_checkin(key, http)
