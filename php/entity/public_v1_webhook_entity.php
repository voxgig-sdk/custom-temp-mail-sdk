...
// CustomTempMail SDK PublicV1Webhook entity
...
class PublicV1WebhookEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PublicV1WebhookEntity($this->_client, $opts);
    }

    /**
     * @param PublicV1Webhook|array $args PublicV1Webhook data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CustomTempMailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PublicV1Webhook|array The current PublicV1Webhook data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PublicV1Webhook fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CustomTempMailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PublicV1Webhook fields).
     */
    ...
    /**
     * List PublicV1Webhook items matching the given filter.
     *
     * @param PublicV1WebhookListMatch|array|null $reqmatch Match filter (any subset
     *   of PublicV1Webhook fields) as an assoc-array; PublicV1WebhookListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PublicV1Webhook[]|array A list of PublicV1Webhook items as assoc-arrays at
     *   the SDK boundary; throws CustomTempMailError on failure (item-5 convention).
     */
    ...
    /**
     * Create a new PublicV1Webhook.
     *
     * @param PublicV1WebhookCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed PublicV1WebhookCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PublicV1Webhook|array The created PublicV1Webhook as an assoc-array at the
     *   SDK boundary; throws CustomTempMailError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = CustomTempMailHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * Remove an PublicV1Webhook matching the given criteria.
     *
     * @param PublicV1WebhookRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; PublicV1WebhookRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PublicV1Webhook|array The removed PublicV1Webhook as an assoc-array at the
     *   SDK boundary; throws CustomTempMailError on failure (item-5 convention).
     */
