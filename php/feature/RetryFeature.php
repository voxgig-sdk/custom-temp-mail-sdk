...
// CustomTempMail SDK retry feature
...
class CustomTempMailRetryFeature extends CustomTempMailBaseFeature
{
    ...
    public function init(CustomTempMailContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CustomTempMailContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CustomTempMailContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CustomTempMailContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
