...
// CustomTempMail SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CustomTempMailTestFeature extends CustomTempMailBaseFeature
{
    ...
    public function init(CustomTempMailContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CustomTempMailContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CustomTempMailParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CustomTempMailContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CustomTempMailContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CustomTempMailParam::call($ctx, $k);
