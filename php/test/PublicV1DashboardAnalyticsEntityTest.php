...
class PublicV1DashboardAnalyticsEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMTEMPMAIL_TEST_PUBLIC_V__DASHBOARD_ANALYTICS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function public_v1_dashboard_analytics_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CUSTOMTEMPMAIL_TEST_PUBLIC_V__DASHBOARD_ANALYTICS_ENTID");
