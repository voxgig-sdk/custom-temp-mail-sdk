...
class UsageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOM_TEMP_MAIL_TEST_USAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function usage_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CUSTOM_TEMP_MAIL_TEST_USAGE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CUSTOM_TEMP_MAIL_TEST_USAGE_ENTID"]);
