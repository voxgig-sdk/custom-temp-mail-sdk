...
// CustomTempMail SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = CustomTempMailOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(CustomTempMailOmni::structfix($now, Struct::undef()))
                        !== json_encode(CustomTempMailOmni::structfix($val, Struct::undef()))) {
