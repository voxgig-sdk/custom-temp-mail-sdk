...
class CustomTempMailMakeError
{
    public static function call(?CustomTempMailContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CustomTempMailContext([], null);
        }
        $op = $ctx->op ?? new CustomTempMailOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CustomTempMailResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CustomTempMailError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CustomTempMailError('', $msg, $ctx);
        ...
        if ($err instanceof CustomTempMailError) {
