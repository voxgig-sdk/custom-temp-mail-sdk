# custom-temp-mail-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CustomTempMail SDK as
two agent tools — `custom-temp-mail_list` and `custom-temp-mail_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/custom-temp-mail-mcp)
make build

# 2. Provide credentials via the environment
export CUSTOM_TEMP_MAIL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user custom-temp-mail \
  -- /absolute/path/to/custom-temp-mail-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./custom-temp-mail-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// custom-temp-mail_list: first page of records
{ "entity": "custom_domain" }
{ "entity": "custom_domain", "query": { } }

// custom-temp-mail_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/custom-temp-mail-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user custom-temp-mail \
     -- "$PWD"/dist/*/custom-temp-mail-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `custom-temp-mail_list` and `custom-temp-mail_load` tools now appear
   in new sessions. Ask the agent to *"list custom_domain using custom-temp-mail"*
   and it calls `custom-temp-mail_list` with `{"entity":"custom_domain"}`.
...
```sh
export CUSTOM_TEMP_MAIL_APIKEY=sk_live_xxx            # API key
export CUSTOM_TEMP_MAIL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `custom-temp-mail_list` tool
...
### Call the `custom-temp-mail_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `custom-temp-mail_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `custom-temp-mail_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CUSTOM_TEMP_MAIL_APIKEY` | API key sent with every request. |
| `CUSTOM_TEMP_MAIL_BASE` | Optional override of the API base URL. |

### Entities

The 14 entities valid as the `entity` argument:

custom_domain | custom_domain_verify | domain | domains_all | inbox | men | message | otp | plan | public_v1_dashboard_analytics | public_v1_inbox | public_v1_message | public_v1_webhook | usage

### Smoke test via HTTP (raw JSON-RPC)

```sh
./custom-temp-mail-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"custom-temp-mail_load","arguments":{"entity":"inbox","query":{"id":1}}}}'
```
