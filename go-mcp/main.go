...
func main() {
	...
	// Configure from the environment: CUSTOM_TEMP_MAIL_APIKEY carries the API key and
	// CUSTOM_TEMP_MAIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CUSTOM_TEMP_MAIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CUSTOM_TEMP_MAIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCustomTempMailSDK(opts)
	server := mcp.NewServer(
		&mcp.Implementation{
			Name:    "custom-temp-mail",
			Version: "0.0.0",
		},
		nil,
	)
