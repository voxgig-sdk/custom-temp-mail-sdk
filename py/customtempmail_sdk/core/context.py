...
from customtempmail_sdk.core.control import CustomTempMailControl
...
from customtempmail_sdk.core.helpers import get_ctx_prop, to_map
...
class CustomTempMailContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = CustomTempMailControl()
        ...
        if isinstance(sp, CustomTempMailSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CustomTempMailResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CustomTempMailResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return CustomTempMailOperation({})
    ...
    def make_error(self, code, msg):
        return CustomTempMailError(code, msg, self)
