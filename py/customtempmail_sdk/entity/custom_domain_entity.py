...
from customtempmail_sdk.customtempmail_types import (
    CustomDomain,
    CustomDomainListMatch,
    CustomDomainCreateData,
    CustomDomainRemoveMatch,
)
...
class CustomDomainEntity:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[CustomDomain]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.CustomDomain().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> CustomDomain:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.CustomDomain().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
