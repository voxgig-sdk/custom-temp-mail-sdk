...
class PublicV1MessageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PublicV1MessageEntity(self._client, opts)
    ...
    def data_get(self) -> PublicV1Message:
    ...
    def match_get(self) -> PublicV1Message:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PublicV1Message:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PublicV1Message().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PublicV1Message:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PublicV1Message().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
