...
from customtempmail_sdk.customtempmail_types import (
    PublicV1Webhook,
...
class PublicV1WebhookEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PublicV1WebhookEntity(self._client, opts)
    ...
    def data_get(self) -> PublicV1Webhook:
    ...
    def match_get(self) -> PublicV1Webhook:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PublicV1Webhook]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PublicV1Webhook().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: PublicV1WebhookCreateData, ctrl=None) -> PublicV1Webhook:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PublicV1Webhook:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PublicV1Webhook().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
