...
class PublicV1InboxEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PublicV1InboxEntity(self._client, opts)
    ...
    def data_get(self) -> PublicV1Inbox:
    ...
    def match_get(self) -> PublicV1Inbox:
    ...
    def create(self, reqdata: PublicV1InboxCreateData, ctrl=None) -> PublicV1Inbox:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PublicV1Inbox:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PublicV1Inbox().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
