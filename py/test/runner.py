# CustomTempMail SDK test runner
...
class CustomTempMailTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CustomTempMailTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CustomTempMailTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CustomTempMailTestRunner.getenv("CUSTOM_TEMP_MAIL_TEST_LIVE")
        override = CustomTempMailTestRunner.getenv("CUSTOM_TEMP_MAIL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CustomTempMailTestRunner.getenv(key)
        ...
        explain = CustomTempMailTestRunner.getenv("CUSTOM_TEMP_MAIL_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CustomTempMailTestRunner._test_control is not None:
            return CustomTempMailTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CustomTempMailTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CustomTempMailTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CustomTempMailTestRunner.load_test_control()
...
def load_env_local():
    CustomTempMailTestRunner.load_env_local()


def env_override(m):
    return CustomTempMailTestRunner.env_override(m)


def entity_data(v):
    return CustomTempMailTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CustomTempMailTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CustomTempMailTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CustomTempMailTestRunner.load_test_control()


def live_client_options():
    return CustomTempMailTestRunner.live_client_options()


def live_delay_ms():
    return CustomTempMailTestRunner.live_delay_ms()
