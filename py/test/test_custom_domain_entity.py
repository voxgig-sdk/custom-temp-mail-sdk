...
class TestCustomDomainEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.CustomDomain(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.CustomDomain(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOM_TEMP_MAIL_TEST_CUSTOM_DOMAIN_ENTID JSON to run live")
        client = setup["client"]
...
def _custom_domain_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOM_TEMP_MAIL_TEST_CUSTOM_DOMAIN_ENTID"))
