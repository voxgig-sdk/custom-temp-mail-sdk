...
class TestDomainsAllDirect:

    def test_should_direct_list_domains_all(self):
        setup = _domains_all_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-domains_all", "live" if setup["live"] else "unit")
        if _skip:
...
def _domains_all_direct_setup(mockres):
