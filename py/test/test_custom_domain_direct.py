...
class TestCustomDomainDirect:
    ...
    def test_should_direct_list_custom_domain(self):
        setup = _custom_domain_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-custom_domain", "live" if setup["live"] else "unit")
        if _skip:
