...
class TestPublicV1InboxEntity:

    def test_should_create_instance(self):
        testsdk = CustomTempMailSDK.test(None, None)
        ent = testsdk.PublicV1Inbox(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _public_v1_inbox_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "public_v1_inbox." + _op, "live" if _live else "unit")
            if _skip:
        ...
        # CREATE
        public_v1_inbox_ref01_ent = client.PublicV1Inbox(None)
...
def _public_v1_inbox_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/public_v1_inbox/PublicV1InboxTestData.json")
