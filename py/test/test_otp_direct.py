...
class TestOtpDirect:
    ...
    def test_should_direct_load_otp(self):
        setup = _otp_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-otp", "live" if setup["live"] else "unit")
        if _skip:
