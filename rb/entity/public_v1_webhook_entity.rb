...
class PublicV1WebhookEntity
  ...
  def make
    opts = @_entopts.dup
    PublicV1WebhookEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CustomTempMailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PublicV1Webhook, Hash] the current PublicV1Webhook data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CustomTempMailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PublicV1Webhook fields)
  def match_get
  ...
  #
  # @param reqmatch [PublicV1WebhookListMatch, Hash, nil] match filter (any subset of
  #   PublicV1Webhook fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PublicV1Webhook>, Array] the matching PublicV1Webhook items; raises CustomTempMailError on failure
  ...
  #
  # @param reqdata [PublicV1WebhookCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [PublicV1Webhook, Hash] the created PublicV1Webhook; raises CustomTempMailError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = CustomTempMailHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PublicV1WebhookRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [PublicV1Webhook, Hash] the removed PublicV1Webhook; raises CustomTempMailError on failure
