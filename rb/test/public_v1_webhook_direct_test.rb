...
class PublicV1WebhookDirectTest < Minitest::Test
  def test_direct_list_public_v1_webhook
    setup = public_v1_webhook_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-public_v1_webhook", setup[:live] ? "live" : "unit")
    if _should_skip
...
def public_v1_webhook_direct_setup(mockres)
