...
module CustomTempMailOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if CustomTempMailOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = CustomTempMailOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| CustomTempMailOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || CustomTempMailOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = CustomTempMailSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = CustomTempMailError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { CustomTempMailOmni.errify(err) }
