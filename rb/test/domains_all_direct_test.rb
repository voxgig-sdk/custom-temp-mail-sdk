...
class DomainsAllDirectTest < Minitest::Test
  def test_direct_list_domains_all
    setup = domains_all_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-domains_all", setup[:live] ? "live" : "unit")
    if _should_skip
...
def domains_all_direct_setup(mockres)
