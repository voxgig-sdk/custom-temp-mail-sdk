...
module CustomTempMailUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CustomTempMailResponse.new({})
    ...
    if fetch_err
      ...
      response = CustomTempMailResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CustomTempMailResponse.new(fetched)
    else
