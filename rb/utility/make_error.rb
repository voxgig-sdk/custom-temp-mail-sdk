...
module CustomTempMailUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = CustomTempMailContext.new({}, nil)
    end
    op = ctx.op || CustomTempMailOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || CustomTempMailResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(CustomTempMailError) ? err.msg : err.to_s
    ...
    sdk_err = CustomTempMailError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(CustomTempMailError)
