...
local OtpEntity = {}
OtpEntity.__index = OtpEntity


function OtpEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OtpEntity)
  self._name = "otp"
  self._client = client
...
function OtpEntity:get_name()
  return self._name
end


function OtpEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OtpEntity.new(self._client, opts)
end
...
function OtpEntity:mark_deleted()
  self._deleted = true
end


function OtpEntity:deleted()
  return true == self._deleted
end


function OtpEntity:data_set(args)
  if args ~= nil then
...
function OtpEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OtpEntity:match_set(args)
  if args ~= nil then
...
function OtpEntity:stream(action, args, callopts)
...
---@param reqmatch OtpLoadMatch
---@param ctrl? table
---@return Otp
---@return string? err
function OtpEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function OtpEntity:_run_op(ctx, post_done)
  local utility = self._utility
