...
local PublicV1WebhookEntity = {}
PublicV1WebhookEntity.__index = PublicV1WebhookEntity


function PublicV1WebhookEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicV1WebhookEntity)
  self._name = "public_v1_webhook"
  self._client = client
...
function PublicV1WebhookEntity:get_name()
  return self._name
end


function PublicV1WebhookEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicV1WebhookEntity.new(self._client, opts)
end
...
function PublicV1WebhookEntity:mark_deleted()
  self._deleted = true
end


function PublicV1WebhookEntity:deleted()
  return true == self._deleted
end


function PublicV1WebhookEntity:data_set(args)
  if args ~= nil then
...
function PublicV1WebhookEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicV1WebhookEntity:match_set(args)
  if args ~= nil then
...
function PublicV1WebhookEntity:match_get()
...
function PublicV1WebhookEntity:stream(action, args, callopts)
...
---@param reqmatch PublicV1WebhookListMatch
---@param ctrl? table
---@return PublicV1Webhook[]
---@return string? err
function PublicV1WebhookEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata PublicV1WebhookCreateData
---@param ctrl? table
---@return PublicV1Webhook
---@return string? err
function PublicV1WebhookEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch PublicV1WebhookRemoveMatch
---@param ctrl? table
---@return PublicV1Webhook
---@return string? err
function PublicV1WebhookEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function PublicV1WebhookEntity:_run_op(ctx, post_done)
  local utility = self._utility
