...
local PublicV1InboxEntity = {}
PublicV1InboxEntity.__index = PublicV1InboxEntity


function PublicV1InboxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicV1InboxEntity)
  self._name = "public_v1_inbox"
  self._client = client
...
function PublicV1InboxEntity:get_name()
  return self._name
end


function PublicV1InboxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicV1InboxEntity.new(self._client, opts)
end
...
function PublicV1InboxEntity:mark_deleted()
  self._deleted = true
end


function PublicV1InboxEntity:deleted()
  return true == self._deleted
end


function PublicV1InboxEntity:data_set(args)
  if args ~= nil then
...
function PublicV1InboxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicV1InboxEntity:match_set(args)
  if args ~= nil then
...
function PublicV1InboxEntity:match_get()
...
function PublicV1InboxEntity:stream(action, args, callopts)
...
---@param reqdata PublicV1InboxCreateData
---@param ctrl? table
---@return PublicV1Inbox
---@return string? err
function PublicV1InboxEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch PublicV1InboxRemoveMatch
---@param ctrl? table
---@return PublicV1Inbox
---@return string? err
function PublicV1InboxEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function PublicV1InboxEntity:_run_op(ctx, post_done)
  local utility = self._utility
