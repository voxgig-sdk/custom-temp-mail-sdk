...
local DomainsAllEntity = {}
DomainsAllEntity.__index = DomainsAllEntity


function DomainsAllEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DomainsAllEntity)
  self._name = "domains_all"
  self._client = client
...
function DomainsAllEntity:get_name()
  return self._name
end


function DomainsAllEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DomainsAllEntity.new(self._client, opts)
end
...
function DomainsAllEntity:mark_deleted()
  self._deleted = true
end


function DomainsAllEntity:deleted()
  return true == self._deleted
end


function DomainsAllEntity:data_set(args)
  if args ~= nil then
...
function DomainsAllEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DomainsAllEntity:match_set(args)
  if args ~= nil then
...
function DomainsAllEntity:match_get()
...
function DomainsAllEntity:stream(action, args, callopts)
...
---@param reqmatch DomainsAllListMatch
---@param ctrl? table
---@return DomainsAll[]
---@return string? err
function DomainsAllEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DomainsAllEntity:_run_op(ctx, post_done)
  local utility = self._utility
