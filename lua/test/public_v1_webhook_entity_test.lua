...
describe("PublicV1WebhookEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:PublicV1Webhook(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:PublicV1Webhook(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:PublicV1Webhook(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = public_v1_webhook_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "public_v1_webhook." .. _op, _live and "live" or "unit")
    ...
    -- CREATE
    local public_v1_webhook_ref01_ent = client:PublicV1Webhook(nil)
    local public_v1_webhook_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local public_v1_webhook_ref01_match = {}

    local public_v1_webhook_ref01_list_result, err = public_v1_webhook_ref01_ent:list(public_v1_webhook_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(public_v1_webhook_ref01_list_result)
    ...
    -- REMOVE
    local public_v1_webhook_ref01_match_rm0 = {
      id = public_v1_webhook_ref01_data["id"],
    }
    local _, err = public_v1_webhook_ref01_ent:remove(public_v1_webhook_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local public_v1_webhook_ref01_match_rt0 = {}

    local public_v1_webhook_ref01_list_rt0_result, err = public_v1_webhook_ref01_ent:list(public_v1_webhook_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(public_v1_webhook_ref01_list_rt0_result)
...
function public_v1_webhook_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/public_v1_webhook/PublicV1WebhookTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read public_v1_webhook test data: " .. entity_data_file)
  end
