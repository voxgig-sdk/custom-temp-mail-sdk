# custom-temp-mail-cli

boru-driven command-line client **and** interactive REPL for the CustomTempMail
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/custom-temp-mail-cli)
make build

# 2. See usage (words, entities, env vars)
./custom-temp-mail-cli --help

# 3. Provide credentials once, via the environment
export CUSTOM_TEMP_MAIL_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./custom-temp-mail-cli list custom_domain
./custom-temp-mail-cli list custom_domain_verify

# 5. Override the API base URL for a single call
CUSTOM_TEMP_MAIL_BASE=https://api.example.com ./custom-temp-mail-cli list custom_domain

# 6. No arguments -> interactive REPL
./custom-temp-mail-cli
custom-temp-mail> list custom_domain
custom-temp-mail> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/custom-temp-mail-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/custom-temp-mail-cli list custom_domain
   ```
...
Run with no arguments to open a REPL (prompt `custom-temp-mail>`). Each line is
evaluated as its own boru expression:

```text
$ ./custom-temp-mail-cli
custom-temp-mail> list custom_domain
custom-temp-mail> /help
custom-temp-mail> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `custom_domain`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/custom-temp-mail-cli`. |
...
(`./custom-temp-mail-cli <expr>`) and interactively (the REPL), and expressions compose the
